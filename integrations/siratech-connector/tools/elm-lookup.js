...
const DOB = (args[1] || '').trim();   // YYYY-MM-DD, optional (needed for Nphies registry)
...
(async () => {
  ...
  const elmBodies = DOB ? [
    ...
    { IdNumber: ID, HospitalId: HOSPITAL_ID },
    { idNumber: ID },
    { nationalId: ID, hospitalId: HOSPITAL_ID },
    { saudiId: ID, hospitalId: HOSPITAL_ID },
  ];
  let elmHit = null;
  ...
  // 2) Nphies patient registry (needs DOB) — deterministic GET with known params.
  if (DOB) {
    ...
    let r; try { r = await call(tok, 'GET', ep); console.log(`\n  NPHIESPatientRegistry → HTTP ${r.status}`);
      console.log(JSON.stringify(r.json ?? r.text, null, 2).slice(0, 2000)); }
    catch (e) { console.log('\n  NPHIESPatientRegistry → ERROR ' + String(e.message || e).slice(0, 80)); }
  } else {
    console.log('\n  (skip Nphies registry — pass a DOB as the 2nd arg to try it: node elm-lookup.js ' + ID + ' 1970-05-01 --yes)');
  }
  console.log('\n── done ──');
  process.exit(0);
