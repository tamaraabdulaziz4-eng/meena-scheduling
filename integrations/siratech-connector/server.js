...
app.get('/diag/panel-dates', requireAuth, async (req, res) => {
  try {
    ...
    const dupInv = [], dupBillSvc = [], nullOrderRows = [];
    for (const r of rows) {
      ...
      const cancelled = cancelRe.test(st);
      const hasOrder = !!r.proposedDate;
      if (!hasOrder) { nullOrderDate += 1; if (nullOrderRows.length < 20) nullOrderRows.push({ mrno: String(r.mrno || ''), service: _risServiceOf(r), billNo: r.billNo != null ? r.billNo : null, billDate: r.billDate || null, billingStatus: r.billingStatus || null }); }
      if (!r.billDate) nullBillDate += 1;
      if (hasOrder) withOrderDate += 1;
      if (hasOrder && !cancelled) withOrderNotCancelled += 1;
      if (String(r.billingStatus) === 'Billed' && !cancelled) billedNotCancelled += 1;
    ...
    return res.json({ ok: true, build: CONNECTOR_BUILD, site, from, to,
      ...
      // candidates: which exclusion rule reproduces the report's count.
      candidates: { withOrderDate, withOrderDateNotCancelled: withOrderNotCancelled, billedStatusNotCancelled: billedNotCancelled },
...
app.get('/diag/done-signal', requireAuth, async (req, res) => {
  try {
    ...
    for (const it of items) {
      ...
      const g = byDay.get(day) || { date: day, ordered: 0, hisReported: 0, pacs: 0, scannedNoPacs: 0, dexa: 0, notDone: 0 };
      g.ordered += 1;
      const isDexa = NONPACS.test(`${it.exam || ''} ${it.modality || ''}`);
      // it.scanned = Siratech recorded an exam start/end (the scan was PERFORMED on the modality),
      // even if it never reached PACS — the manual/non-integrated period signal.
      if (it.hisReported) { hisReported += 1; g.hisReported += 1; }
      else if (['imaged', 'draft', 'reported'].includes(it.stage)) { pacs += 1; g.pacs += 1; }
      else if (it.scanned || it.examEndAt || it.examStartAt) { scannedNoPacs += 1; g.scannedNoPacs += 1; }
    ...
    const donePlusScanned = hisReported + pacs + scannedNoPacs;
    return res.json({ ok: true, build: CONNECTOR_BUILD, site: site || 'all', from, to, matchAfterH,
      total, hisReported, pacs, scannedNoPacs, dexa, notDone,
      doneNow: hisReported + pacs, doneNowPct: total ? Math.round((hisReported + pacs) / total * 100) : 0,
